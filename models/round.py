...
class Round:
    ...
    def from_dict(cls, round_dict, players_dict):
        ...
        start_date = datetime.strptime(round_dict["start_date"], '%Y-%m-%d %H:%M:%S')
        end_date = datetime.strptime(round_dict["end_date"], '%Y-%m-%d %H:%M:%S') if round_dict["end_date"] else None
        matches = [Match.from_dict(match_dict, players_dict) for match_dict in round_dict["match_list"]]
        round_obj = cls(name)
